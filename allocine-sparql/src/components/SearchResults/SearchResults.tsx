...
export default function SearchResults (props : ISearchResultsProps) {
    ...
    function _sparqlSearch() {
        ...
        serviceInstance.fetchMovie({"title": searchString, "beforeYear": yearMin, "afterYear": yearMax}, {size:15, page:pageNumber}).then((result) => {
            ...
            <div className={styles.center}>
                {
                    Array.from(Array(maxPage - minPage + 1).keys()).map((_, i) => {
                        ...
                        return (
                            <a onClick={_ => handleChangeSearchPage((minPage + i))}> {minPage + i} </a>
                        )
                    })
                }
            </div>
